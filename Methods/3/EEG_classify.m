...
right_processed=load('3/rightBandpass0190Nothch4852');
left_processed=load('3/leftBandpass0190Nothch4852');
...
right_C4_features=zeros(127,33);
right_C3_features=zeros(127,33);
left_C4_features=zeros(127,33);
left_C3_features=zeros(127,33);


% Get the Multiscale Wavelet Transform features(Energy, Variance, and
% Waveform Length) 
for i=1:size(right_C4_choped,1)
    right_C4_features(i,:)=getmswtfeat(right_C4_choped(i,:)',64,1000,1000);
    right_C3_features(i,:)=getmswtfeat(right_C3_choped(i,:)',64,1000,1000);
    left_C4_features(i,:)=getmswtfeat(left_C4_choped(i,:)',64,1000,1000);
    left_C3_features(i,:)=getmswtfeat(left_C3_choped(i,:)',64,1000,1000);
end
